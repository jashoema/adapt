...
async def run(deps: ActionAnalyzerDependencies, debug_mode: bool = False, logger: Optional[Any] = None) -> RunContext:
    """
    Run the action_analyzer agent to analyze the output of a network device command.
    
    Args:
        deps: The dependencies containing executor output, action plan, fault summary, and current step
        debug_mode: Whether to log detailed debugging information
        logger: Optional logger instance to use for logging
    ...
    if debug_mode and logger:
        ...
        logger.info("Action Analyzer Agent System Prompt", extra={
            "system_prompt": ACTION_ANALYZER_SYSTEM_PROMPT
        })
